...
test("serves the cockpit and exposes authenticated operational data", async (t) => {
  ...
  assert.match(cockpitHtml, /Cockpit interno/)
  ...
  assert.match(cockpitScript.headers.get("content-type"), /javascript/)
  assert.match(await cockpitScript.text(), /function groupConversations/)

  const cockpitStyles = await fetch(`${baseUrl}/cockpit/styles.css`)
  assert.equal(cockpitStyles.status, 200)
  assert.match(cockpitStyles.headers.get("content-type"), /text\/css/)
  assert.match(await cockpitStyles.text(), /\.conversation-workspace/)
